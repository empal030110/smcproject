...
-----------------
SELECT * FROM MEMBER WHERE EMAIL LIKE '%gmail.com%' AND LENGTH(EMAIL) > 11;
...
-- 조건중 AND OR 를 분리하고 싶을때
-- (조건1 AND 조건2) OR (조건3 AND 조건4)
-- 세번째 문제
-- 이메일이 GMAIL 이면서 길이가 12글자 이상이거나,
-- 이메일이 NAVER.COM을 사용하면서 10글자 이상인 조건에 해당하는 멤버를 찾으세요
SELECT * FROM MEMBER
WHERE
    (EMAIL LIKE '%gmail.com%' AND LENGTH(EMAIL) > 11) 
    OR
    (EMAIL LIKE '%naver.com%' AND LENGTH(EMAIL) > 9);
...
-- 결제 금액이 1,000,000 이상이면서 구매 개수(order_count)가 10개 이하인 주문이거나,
-- 결제 금액이 100,000 이하이면서 구매 개수가 10 이상인 주문을 찾으세요
...
SELECT * FROM MEMBER
WHERE 
    ((EMAIL LIKE'%gamil.com%' AND ADDRESS LIKE'%서울시%' AND LENGTH(PASSWORD) >= 10)
    OR
    (EMAIL LIKE'%naver.com%' AND PHON LIKE'%1%'))
    AND AGE <= 100;
...
SELECT 
    ...
    PAYMENT_HISTORY.ORDER_DATE AS 구매날짜
FROM 
